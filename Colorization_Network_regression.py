...
from project_nn import ConvReLU, DeConvReLU, \
    BatchNorm, Conv, Colorization_Softmax, Colorization_Decoding, Colorization_PriorBoost, dec_net_out_to_rgb
...
class colorization(object):
    ...
    def build_model(self,
        ...
        self.data_ab = self.y.reshape((batch_size, 2, 256*256))
        ...
        self.conv8_ab_upsampled = abstract_conv.bilinear_upsampling(self.conv8_ab.output, 4)
        ...
        self.params = self.convrelu1_1.params + self.convrelu1_2.params + self.bn_1.params + self.convrelu2_1.params + self.convrelu2_2.params + \
                      ...
                      self.bn_7.params + self.convrelu8_1.params + self.convrelu8_2.params + self.convrelu8_3.params + self.conv8_ab.params

        # self.data_ab_enc_for_cost_func = self.data_ab_enc.reshape((batch_size, 4096, 313))

        self.cost = (256*256)*T.mean(T.sqrt(T.square(T.flatten(self.data_ab.transpose(0,2,1) - self.conv8_ab_upsampled.transpose(0,2,1)))))
    ...
    def train_network(
        ...
        if not os.path.isdir(dir_name):
            ...
            prepare_image_sets(dir_name, batch_size=200)
        self.train_set_x, self.train_set_y = load_data(dir_name, theano_shared=True, ds=ds_rate, batch_ind=batch_ind,
                                                       batch_num=batch_num)
    ...
    def test_network(
        ...
        self.test_set_x, self.test_set_y = load_data(dir_name, theano_shared=True, ds=ds_rate, batch_ind=batch_ind,
                                                     batch_num=1)
        ...
        self.output_model = theano.function(
            [self.index],
            [self.bw_input, self.conv8_ab_upsampled, self.data_ab],
            givens={
